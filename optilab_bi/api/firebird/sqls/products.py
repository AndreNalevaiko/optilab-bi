def products_trans():
    return """
    select 'geral_trans',sum(nfp.nfpqtdade) as qtdade, sum(nfp.nfpqtdade * nfp.nfpunitliquido) as vr_venda_bruta,
...
def products_varilux():
    return """
    select 'geral_vlx', sum(nfp.nfpqtdade) as qtdade, sum(nfp.nfpqtdade * nfp.nfpunitliquido) as vr_venda_bruta,
    nfs.empcodigo, EXTRACT(YEAR FROM nfs.nfdtemis) ANO, EXTRACT(MONTH FROM nfs.nfdtemis) MES
    from notas nfs
    left join nfpro    nfp on nfs.nfcodigo  = nfp.nfcodigo
                        and nfs.empcodigo = nfp.empcodigo
    ...
    group by   ANO,  MES,  nfs.empcodigo
    UNION ALL
    select 'vlx_s',sum(nfp.nfpqtdade) as qtdade, sum(nfp.nfpqtdade * nfp.nfpunitliquido) as vr_venda_bruta,
    nfs.empcodigo, EXTRACT(YEAR FROM nfs.nfdtemis) ANO, EXTRACT(MONTH FROM nfs.nfdtemis) MES
    from notas nfs
    left join nfpro    nfp on nfs.nfcodigo  = nfp.nfcodigo
                        and nfs.empcodigo = nfp.empcodigo
...
def products_kodak():
    return """
    select 'geral_kodak',sum(nfp.nfpqtdade) as qtdade, sum(nfp.nfpqtdade * nfp.nfpunitliquido) as vr_venda_bruta,
    nfs.empcodigo, EXTRACT(YEAR FROM nfs.nfdtemis) ANO, EXTRACT(MONTH FROM nfs.nfdtemis) MES
    from notas nfs
    left join nfpro    nfp on nfs.nfcodigo  = nfp.nfcodigo
                        and nfs.empcodigo = nfp.empcodigo
...
def products_itop():
    return """
    select 'geral_itop',sum(nfp.nfpqtdade) as qtdade, sum(nfp.nfpqtdade * nfp.nfpunitliquido) as vr_venda_bruta,
    nfs.empcodigo, EXTRACT(YEAR FROM nfs.nfdtemis) ANO, EXTRACT(MONTH FROM nfs.nfdtemis) MES
    from notas nfs
    left join nfpro    nfp on nfs.nfcodigo  = nfp.nfcodigo
                        and nfs.empcodigo = nfp.empcodigo
...
def products_crizal():
    return """
    select 'geral_crizal',sum(nfp.nfpqtdade) as qtdade, sum(nfp.nfpqtdade * nfp.nfpunitliquido) as vr_venda_bruta,
    nfs.empcodigo, EXTRACT(YEAR FROM nfs.nfdtemis) ANO, EXTRACT(MONTH FROM nfs.nfdtemis) MES
    from notas nfs
    left join nfpro    nfp on nfs.nfcodigo  = nfp.nfcodigo
                        and nfs.empcodigo = nfp.empcodigo
